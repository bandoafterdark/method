...
# Define functions as before, but remove colorama and print statements, and replace with GUI elements.
...
class TikTokChangerApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("TikTok Username Changer")
    ...
    def create_widgets(self):
        # Session ID input
        tk.Label(self, text="Session ID:").pack(pady=5)
        self.session_id_entry = tk.Entry(self, width=50)
        self.session_id_entry.pack(pady=5)
        
        # New Username input
        tk.Label(self, text="New Username:").pack(pady=5)
        self.new_username_entry = tk.Entry(self, width=50)
        self.new_username_entry.pack(pady=5)
        
        # Method selection
        tk.Label(self, text="Method:").pack(pady=5)
        self.method_var = tk.StringVar(value="1")
        tk.Radiobutton(self, text="User Method", variable=self.method_var, value="1").pack()
        tk.Radiobutton(self, text="Redirect Method", variable=self.method_var, value="2").pack()

        # Submit button
        tk.Button(self, text="Change Username", command=self.change_username).pack(pady=20)
    ...
    def change_username(self):
        ...
        if not session_id or not new_username:
            messagebox.showerror("Input Error", "Please provide both Session ID and New Username.")
            return
        ...
        if user == "None":
            messagebox.showerror("Error", "Invalid session ID or other error.")
            return
        ...
        if not change_result:
            self.status_label.config(text=f"Username successfully changed to: {new_username}")
            messagebox.showinfo("Success", f"Username successfully changed to: {new_username}")
        else:
            messagebox.showerror("Error", change_result)

if __name__ == "__main__":
    app = TikTokChangerApp()
    app.mainloop()
